...
def test_eda():
    ...
    assert list(summary.columns) == ['min','variance','min','quantile25','quantile50','quantile75','max']), "The summary dataframe's columnnames are not as expected"
    ...
    # mean values:
    assert summary.mean[0] == y_mean, "Mean of response(y) is wrong"
    ...
    # variance values:
    assert summary.variance[0] == y_var, "variance of response(y) is wrong"
